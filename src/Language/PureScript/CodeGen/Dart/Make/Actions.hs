...
backendMakeActions outputDir filePathMap foreigns usePrefix =
  ...
  targetFilename mn = \case
    JS -> outputFilename mn "index.dart"
    JSSourceMap -> outputFilename mn "index.dart.map"
  ...
  codegen m docs exts = do
    ...
    when (S.member JS codegenTargets) $ do
      ...
      let sourceMaps = S.member JSSourceMap codegenTargets
          (pjs, mappings) =
            if sourceMaps
              then Dart.prettyPrintJSWithSourceMaps rawJs
              else (Dart.prettyPrintJS rawJs, [])
          ...
          mapRef =
            if sourceMaps
                then "//# sourceMappingURL=index.dart.map\n"
                else ""
      lift $ do
  ...
  ffiCodegen m = do
    ...
    when (S.member JS codegenTargets) $ do
      ...
      for_ (mn `M.lookup` foreigns) $ \path ->
        copyFile path (outputFilename mn "foreign.dart")
  ...
  genSourceMap dir mapFile extraLines mappings = do
    let pathToDir = iterate (".." </>) ".." !! length (splitPath $ normalise outputDir)
        sourceFile = case mappings of
                      ...
                      _ -> Nothing
    let rawMapping = SourceMapping { smFile = "index.dart", smSourceRoot = Nothing, smMappings =
